...
public class UsosApiArchTest {
                    ...
                    .should(new ArchCondition<JavaClass>("return only our classes, or java classes") {
                        ...
                        public void check(JavaClass javaClass, ConditionEvents conditionEvents) {
                            ...
                            var badMethods = javaClass.getMethods()
                                    ...
                                    .filter(method -> !(method.getRawReturnType().isPrimitive() || method.getRawReturnType().getPackageName().contains("dev.wms.usos4j")
                            ...
                            for (var method : badMethods) {
                                message.append("%s in %s return %s which resides in package %s but should reside in %s instead "
                                        .formatted(method.getName(), javaClass.getSimpleName(), method.getReturnType().getName(), method.getRawReturnType().getPackageName(), "dev.wms.usos4j.*"));
                            }
    ...
    private final ArchRule public_method_names_should_not_contain_get_or_set =
            methods()
                    .that().areDeclaredInClassesThat().resideInAPackage("dev.wms.usos4j.api.*")
                    .and().arePublic()
                    .should()
    ...
    private GivenClassesConjunction apiClasses() {
        return classes()
                .that().resideInAPackage("dev.wms.usos4j.api.*");
    }
